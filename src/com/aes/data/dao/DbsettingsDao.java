 ...
 * @see com.aes.data.dao.Dbsettings
 * @author Hibernate Tools
 */
public class DbsettingsDao {

	private static final Log log = LogFactory.getLog(DbsettingsDao.class);
	...
	public Dbsettings merge(Dbsettings detachedInstance) {
		...
		try {
			Dbsettings result = (Dbsettings) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Dbsettings findById(int id) {
		...
		try {
			Dbsettings instance = (Dbsettings) sessionFactory.getCurrentSession().get("com.aes.data.dao.Dbsettings",
					id);
			if (instance == null) {
	...
	public List<Dbsettings> findByExample(Dbsettings instance) {
		...
		try {
			List<Dbsettings> results = (List<Dbsettings>) sessionFactory.getCurrentSession()
					.createCriteria("com.aes.data.dao.Dbsettings").add(create(instance)).list();
