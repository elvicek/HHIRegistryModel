 ...
 * @see com.aes.data.dao.SurveyReadings
 * @author Hibernate Tools
 */
public class SurveyReadingsDao {

	private static final Log log = LogFactory.getLog(SurveyReadingsDao.class);
	...
	public SurveyReadings merge(SurveyReadings detachedInstance) {
		...
		try {
			SurveyReadings result = (SurveyReadings) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public SurveyReadings findById(int id) {
		...
		try {
			SurveyReadings instance = (SurveyReadings) sessionFactory.getCurrentSession()
					.get("com.aes.data.dao.SurveyReadings", id);
			if (instance == null) {
	...
	public List<SurveyReadings> findByExample(SurveyReadings instance) {
		...
		try {
			List<SurveyReadings> results = (List<SurveyReadings>) sessionFactory.getCurrentSession()
					.createCriteria("com.aes.data.dao.SurveyReadings").add(create(instance)).list();
